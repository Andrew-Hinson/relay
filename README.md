...
The Debezium plugin ZIP must include the [MSK config-providers JAR](https://github.com/aws-samples/msk-config-providers/releases). `--msk-bootstrap-servers` must be the IAM listeners (port 9098). The Connect role needs `secretsmanager:GetSecretValue` and `DescribeSecret` on the runtime secret (Instance Secret on attach; Apply grants the RDS-managed secret on create), plus Cluster-level MSK Connect internals (`__amazon_msk_connect_*`). Apply attaches `kafka-cluster` on `{prefix}*` topics and groups to that role. Connect VPC needs a path to Secrets Manager. The Apply caller uses the default AWS credential chain for MSK IAM (topics), `secretsmanager:GetSecretValue`, and `iam:PutRolePolicy` on the Connect role.
