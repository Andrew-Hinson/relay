...
func TestRenderTfvars_omitsPassword(t *testing.T) {
	...
	if !strings.Contains(out, "cdc_user = \"acme_acmedb_cdc\"") {
		t.Fatalf("missing cdc_user: %s", out)
	}
	if strings.Contains(out, "secret_name") || strings.Contains(out, "secret_user_key") {
		t.Fatalf("Secrets Manager still in tfvars: %s", out)
	}
}
...
func TestRenderTfvars_rdsUsernameIsInstanceNotConfigRole(t *testing.T) {
	...
	if strings.Contains(out, "rds_username = \"acme_acmedb_cdc\"") {
		t.Fatal("RDS master must not be the CDC role")
	}
	if strings.Contains(out, "rds_username = \"acme_acmedb\"") {
		t.Fatal("RDS master must not be the owner role")
	}
}
...
func TestRenderTfvars_scopesConnectIAMByPrefix(t *testing.T) {
	...
	out := renderTfvars(plan, clusterEnv{
		...
		ConnectSourceBoundaryARN: "arn:aws:iam::000000000000:policy/relay-connect-source-boundary",
		ConnectWorkerPolicyARN:   "arn:aws:iam::000000000000:policy/relay-connect-worker",
	})
	...
	if !strings.Contains(out, "connect_source_boundary_arn = \"arn:aws:iam::000000000000:policy/relay-connect-source-boundary\"") {
		t.Fatalf("missing source boundary: %s", out)
	}
	if !strings.Contains(out, "connect_worker_policy_arn = \"arn:aws:iam::000000000000:policy/relay-connect-worker\"") {
		t.Fatalf("missing worker policy: %s", out)
	}
...
func TestParseApplyFlags_sourceRoleFromEnv(t *testing.T) {
	t.Setenv("RELAY_CONNECT_SOURCE_BOUNDARY_ARN", "arn:aws:iam::000000000000:policy/boundary")
	t.Setenv("RELAY_CONNECT_WORKER_POLICY_ARN", "arn:aws:iam::000000000000:policy/worker")
	_, env, err := parseApplyFlags([]string{"-f", "x.yaml"})
	if err != nil {
		t.Fatal(err)
	}
	if env.ConnectSourceBoundaryARN != "arn:aws:iam::000000000000:policy/boundary" {
		t.Fatalf("got boundary %q", env.ConnectSourceBoundaryARN)
	}
	if env.ConnectWorkerPolicyARN != "arn:aws:iam::000000000000:policy/worker" {
		t.Fatalf("got worker %q", env.ConnectWorkerPolicyARN)
	}
}
...
func TestClusterEnv_validateRequiresSourceBoundary(t *testing.T) {
	env := validClusterEnv()
	env.ConnectSourceBoundaryARN = ""
	if err := env.validate(false); err == nil || !strings.Contains(err.Error(), "connect-source-boundary-arn") {
		t.Fatalf("got %v", err)
	}
}

func TestClusterEnv_validateRequiresWorkerPolicy(t *testing.T) {
	env := validClusterEnv()
	env.ConnectWorkerPolicyARN = ""
	if err := env.validate(false); err == nil || !strings.Contains(err.Error(), "connect-worker-policy-arn") {
		t.Fatalf("got %v", err)
	}
}
...
func validClusterEnv() clusterEnv {
	return clusterEnv{
		...
		ConnectSourceBoundaryARN: "arn:b",
		ConnectWorkerPolicyARN:   "arn:w",
